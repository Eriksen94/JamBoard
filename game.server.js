    ...
    setTimeout(
      function () {
        if (game_server.messages.length) {
          game_server._onMessage(
            game_server.messages[0].client,
            game_server.messages[0].message
          );
...
game_server.createGame = function (player) {
  ...
  var thegame = {
    id: UUID.v4(), //generate a new id for the game
  ...
  this.log(
    "player " + player.userid + " created a game with id " + player.game.id
  );
