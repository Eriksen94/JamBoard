...
io.on("connection", (socket) => {
  ...
  const ids = { player: socket.id, host: crypto.randomBytes(2).toString("hex") };
  ...
  socket.on("game_setup", (gameParams) => {
    ...
    if(roomInd != -1){
      ...
      console.log("room not found to create game");
    }
  });
  ...
  socket.on("move_played", (moveParams) => {
    ...
    if(roomInd != -1){
      ...
      const movePlayed = playMoveInRoom(socket.id, moveParams.row, moveParams.col, roomInd);
      if(movePlayed){
...
function initRoomGameData(params, ind){
  ...
  for(let j = 0; j < playersInGame; j++){
    ...
    let p = new Player(rooms[ind].clientsInLobby[k], order[j], colourPicker(j), 0);
...
//curate the list of colours to be assigned during player creation
//returns hex code str "#XXXXXX"
function colourPicker(pick){
  const colours = [
    "#ec1c27",
    "#4ebc85",
    "#f6881f",
  ...
  if(pick >= colours.length) pick = colours.length-1;
...
function playMoveInRoom(playerNum, row, col, roomInd){
  ...
  let matchedPlayer = 0;
  for(let i = 0; i < rooms[roomInd].game.players.length; i++){
    if(turn === rooms[roomInd].game.players[i].turnOrder && playerNum === rooms[roomInd].game.players[i].uuid){
      turnMatch = true;
      matchedPlayer = i;
    }
  }
  ...
  //check move is valid
  if(turnMatch 
    && row >= 0 && row < rooms[roomInd].game.rows 
    && col >= 0 && col < rooms[roomInd].game.cols){
  ...
  if(validMove){
    ...
    rooms[roomInd].game.players[matchedPlayer].score += scoreMove(row,col, turn, rooms[roomInd].game.boardState);
    rooms[roomInd].game.currentPlayersTurn++;
    //loop turns
    if(rooms[roomInd].game.currentPlayersTurn > rooms[roomInd].game.players.length) rooms[roomInd].game.currentPlayersTurn = 1;
    rooms[roomInd].game.complete = checkIfGameComplete(rooms[roomInd].game.boardState);
  }
